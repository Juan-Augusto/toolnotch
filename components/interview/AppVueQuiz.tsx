...
import { VUE_QUESTIONS } from '@/data/interview/vue-questions'
import type { QuizLevel, QuizState, ExplanationTab, UserAnswer } from '@/lib/interviewTypes'
...
function VueIcon({ size = 48 }: { size?: number }) {
  return (
...
const LEVEL_CARDS: { level: QuizLevel; title: string; desc: string; topics: string[] }[] = [
  {
    level: 'beginner',
    title: 'Beginner',
    desc: '10 questions on Vue fundamentals',
    topics: ['createApp', 'Interpolation', 'v-bind', 'v-if vs v-show', 'v-for & key', 'ref()', 'computed()', 'Props', 'emit()', 'SFCs'],
  },
  {
    level: 'intermediate',
    title: 'Intermediate',
    desc: '10 questions on the Composition API',
    topics: ['ref vs reactive', 'computed vs watch', 'watchEffect', 'provide/inject', 'Slots', 'Lifecycle Hooks', 'Custom v-model', 'Teleport', 'KeepAlive'],
  },
  {
    level: 'advanced',
    title: 'Advanced',
    desc: '10 questions on internals and production systems',
    topics: ['Reactivity Internals', 'Composables', 'Suspense', 'Render Functions', 'Custom Directives', 'Router Guards', 'Pinia', 'shallowRef', 'SSR Hydration', 'Patch Flags'],
  },
]
  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '2.5rem 1.25rem', textAlign: 'center' }}>
      ...
      <h1 style={{ color: 'var(--text-primary)', fontSize: '2rem', fontWeight: 700, margin: '0 0 0.5rem' }}>
        Vue.js Interview Quiz
      </h1>
      <p style={{ color: 'var(--text-muted)', fontSize: '1rem', marginBottom: '2.5rem' }}>
        30 questions across 3 levels — from template syntax and the Composition API to reactivity internals,
        Pinia, SSR hydration, and compiler optimizations. Each answer shows <em>example code</em> and the production best practice.
      </p>
      ...
      <p style={{ color: 'var(--text-muted)', fontSize: '0.78rem', marginTop: '2.5rem' }}>
        Based on Vue.js Docs, Pinia Docs, Vue Router Docs &amp; common frontend interview questions · Keyboard: 1–4 to answer · Enter/→ to advance · Tab to switch tabs
      </p>
    </div>
  )
}
...
type TType = 'kw' | 'cmt' | 'str' | 'num' | 'tag' | 'txt'
...
const JS_KW = new Set([
  ...
  'onBeforeMount', 'onUpdated', 'onActivated', 'onDeactivated', 'provide', 'inject',
  'defineProps', 'defineEmits', 'defineModel', 'defineStore', 'defineAsyncComponent',
  'createApp', 'h', 'Promise', 'resolve', 'reject', 'then',
])

type Token = { t: TType; v: string }

function tokenizeVue(code: string): Token[] {
  const tokens: Token[] = []
  let i = 0
  while (i < code.length) {
    // Line comment
    ...
    if (code[i] === '<' && code[i + 1] === '!' && code[i + 2] === '-' && code[i + 3] === '-') {
      let j = i + 4
      while (j < code.length - 2 && !(code[j] === '-' && code[j + 1] === '-' && code[j + 2] === '>')) j++
      j += 3
      tokens.push({ t: 'cmt', v: code.slice(i, j) })
      i = j
      continue
    ...
    // Tag-like (<template>, </script>, <div>)
    if (code[i] === '<' && /[a-zA-Z/]/.test(code[i + 1] ?? '')) {
      let j = i
      while (j < code.length && code[j] !== '>') j++
      j += 1
      tokens.push({ t: 'tag', v: code.slice(i, j) })
      i = j
      continue
    }
    // Number
...
const TABS: { id: ExplanationTab; label: string }[] = [
  ...
  { id: 'compiled', label: 'Example Code' },
          ...
          return (
            <button
              ...
              style={{
                ...
                animation: shaking === i ? 'vueShake 0.55s ease' : 'none',
              }}
            >
              <span style={{
                flexShrink: 0,
            ...
            {state.activeTab === 'compiled' && (
              q.compiledJS ? (
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: '0.88rem', fontStyle: 'italic', margin: 0 }}>
                  This concept applies at the architecture or configuration level — there is no single code snippet that demonstrates it. See the &ldquo;Why This Answer&rdquo; tab for the full explanation.
                </p>
              )
            )}
...
export function AppVueQuiz() {
  const [state, setState] = useState<QuizState>(INITIAL_STATE)

  const startQuiz = useCallback((level: QuizLevel) => {
    const pool = VUE_QUESTIONS.filter(q => q.id[0] === level[0])
