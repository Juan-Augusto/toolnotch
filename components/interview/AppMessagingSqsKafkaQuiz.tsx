...
import { MESSAGING_SQS_KAFKA_QUESTIONS } from '@/data/interview/messaging-sqs-kafka-questions'
...
function MqIcon({ size = 48 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect width="64" height="64" rx="12" fill="#ff9900" />
      {/* Queue: three horizontal lanes */}
      <rect x="8"  y="14" width="32" height="8" rx="3" fill="#fff" fillOpacity="0.25" stroke="#fff" strokeWidth="1.5" />
      <rect x="8"  y="28" width="40" height="8" rx="3" fill="#fff" fillOpacity="0.25" stroke="#fff" strokeWidth="1.5" />
      <rect x="8"  y="42" width="24" height="8" rx="3" fill="#fff" fillOpacity="0.25" stroke="#fff" strokeWidth="1.5" />
      {/* Arrow right */}
      <path d="M52 32 L58 32 M55 28 L58 32 L55 36" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  )
}
...
const LEVEL_CARDS: { level: QuizLevel; title: string; desc: string; topics: string[] }[] = [
  {
    level: 'beginner',
    title: 'Beginner',
    desc: '10 questions on messaging fundamentals',
    topics: ['Message Queues', 'Producer/Consumer', 'SQS Basics', 'FIFO Queue', 'DLQ', 'At-Least-Once', 'Kafka Intro'],
  },
  {
    level: 'intermediate',
    title: 'Intermediate',
    desc: '10 questions on queue and broker mechanics',
    topics: ['Visibility Timeout', 'Long Polling', 'Kafka Partitions', 'Offsets', 'ISR', 'Exactly-Once', 'Log Compaction'],
  },
  {
    level: 'advanced',
    title: 'Advanced',
    desc: '10 questions on distributed streaming systems',
    topics: ['Log Segments', 'Hot Partitions', 'Back-Pressure', 'Schema Registry', 'Outbox Pattern', 'Kafka Connect', 'MirrorMaker'],
  },
]
  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '2.5rem 1.25rem', textAlign: 'center' }}>
      ...
      <h1 style={{ color: 'var(--text-primary)', fontSize: '2rem', fontWeight: 700, margin: '0 0 0.5rem' }}>
        SQS &amp; Kafka Messaging Quiz
      </h1>
      <p style={{ color: 'var(--text-muted)', fontSize: '1rem', marginBottom: '2.5rem' }}>
        30 questions across 3 levels — covering message queues, partitions, delivery semantics, and advanced streaming
        patterns. Each answer shows the message flow and production best practice.
      </p>
      ...
      <p style={{ color: 'var(--text-muted)', fontSize: '0.78rem', marginTop: '2.5rem' }}>
        Based on AWS Docs, Apache Kafka Docs &amp; DDIA · Keyboard: 1–4 to answer · Enter/→ to advance · Tab to switch tabs
      </p>
    </div>
  )
}
...
const SHELL_KW = new Set([
  'aws','kafka-topics.sh','kafka-console-producer.sh','kafka-console-consumer.sh',
  'kafka-consumer-groups.sh','kafka-dump-log.sh','connect-mirror-maker.sh',
  'curl','docker','echo','ls','for','do','done','BEGIN','COMMIT','ROLLBACK',
  'UPDATE','INSERT','INTO','VALUES','SELECT','FROM','WHERE','CREATE','TABLE',
  'ALTER','DELETE','AND','OR','NOT','NULL','IF','THEN','ELSE','END',
  'const','let','var','function','async','await','return','new','import',
  'require','export','class','interface','type','extends','implements',
  'true','false','undefined','void','never','string','number','boolean',
])
...
function tokenizeCode(code: string): Token[] {
  ...
  let i = 0
  while (i < code.length) {
    // Shell / SQL single-line comments
    if (code[i] === '#' || (code[i] === '-' && code[i + 1] === '-') || (code[i] === '/' && code[i + 1] === '/')) {
      let j = i
    ...
    if (/[a-zA-Z_$]/.test(code[i])) {
      ...
      tokens.push({ t: SHELL_KW.has(w) || SHELL_KW.has(w.toUpperCase()) ? 'kw' : 'txt', v: w })
      i = j
      continue
    }
...
const TABS: { id: ExplanationTab; label: string }[] = [
  ...
  { id: 'compiled', label: 'Message Flow' },
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '2rem 1.25rem' }}>
      ...
      <div style={{ marginBottom: '0.6rem' }}>
        <span style={{ color: '#ff9900', fontSize: '0.75rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
          {q.topic}
        </span>
      </div>
          ...
          return (
            <button
              ...
              style={{
                ...
                animation: shaking === i ? 'mqShake 0.55s ease' : 'none',
              }}
            >
              <span style={{
                flexShrink: 0,
            ...
            {TABS.map(tab => (
              <button
                ...
                style={{
                  ...
                  background: 'none',
                  border: 'none',
                  borderBottom: state.activeTab === tab.id ? '2px solid #ff9900' : '2px solid transparent',
                  color: state.activeTab === tab.id ? '#ff9900' : 'var(--text-muted)',
                  cursor: 'pointer',
                  fontSize: '0.78rem',
                  fontWeight: 600,
            ...
            {state.activeTab === 'compiled' && (
              q.compiledJS ? (
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: '0.88rem', fontStyle: 'italic', margin: 0 }}>
                  This concept applies at the architecture or configuration level — there is no single message flow snippet that demonstrates it. See the &ldquo;Why This Answer&rdquo; tab for the full explanation.
                </p>
              )
            )}
...
function ResultsScreen({ state, onRetry, onHome }: ResultsScreenProps) {
  ...
  const ringColor = score >= 8 ? '#22c55e' : score >= 5 ? '#ff9900' : '#ef4444'
  ...
  return (
    <div style={{ maxWidth: 680, margin: '0 auto', padding: '2.5rem 1.25rem', textAlign: 'center' }}>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center', flexWrap: 'wrap' }}>
        <button
          onClick={onRetry}
          style={{ background: '#ff9900', color: '#0f1117', border: 'none', borderRadius: '8px', padding: '0.65rem 1.5rem', cursor: 'pointer', fontSize: '0.9rem', fontWeight: 700 }}
        >
          Try Again
        </button>
        <button
...
export function AppMessagingSqsKafkaQuiz() {
  const [state, setState] = useState<QuizState>(INITIAL_STATE)

  const startQuiz = useCallback((level: QuizLevel) => {
    const pool = MESSAGING_SQS_KAFKA_QUESTIONS.filter(q => q.id[0] === level[0])
