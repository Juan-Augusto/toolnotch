...
function ArchIcon({ size = 48 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect width="64" height="64" rx="12" fill="#8b5cf6" />
      <rect x="8" y="10" width="20" height="12" rx="3" fill="#fff" fillOpacity="0.85" />
      <rect x="36" y="10" width="20" height="12" rx="3" fill="#fff" fillOpacity="0.85" />
      <rect x="22" y="42" width="20" height="12" rx="3" fill="#fff" fillOpacity="0.85" />
      <line x1="18" y1="22" x2="32" y2="42" stroke="#fff" strokeWidth="2" strokeOpacity="0.6" />
      <line x1="46" y1="22" x2="32" y2="42" stroke="#fff" strokeWidth="2" strokeOpacity="0.6" />
      <line x1="28" y1="22" x2="36" y2="22" stroke="#fff" strokeWidth="2" strokeOpacity="0.4" />
    </svg>
  )
}
...
const LEVEL_CARDS: { level: QuizLevel; title: string; desc: string; topics: string[] }[] = [
  {
    level: 'beginner',
    title: 'Beginner',
    desc: '10 questions on core architectural concepts',
    topics: ['Monolith vs Microservices', 'Docker', 'CI/CD', 'Load Balancer', 'Horizontal Scaling', 'Reverse Proxy', 'Health Checks'],
  },
  {
    level: 'intermediate',
    title: 'Intermediate',
    desc: '10 questions on deployment patterns',
    topics: ['Blue/Green Deploy', 'Kubernetes', 'Service Discovery', 'API Gateway', 'Circuit Breaker', 'Rate Limiting', '12-Factor App'],
  },
  {
    level: 'advanced',
    title: 'Advanced',
    desc: '10 questions on production systems at scale',
    topics: ['GitOps', 'Chaos Engineering', 'Service Mesh', 'Saga Pattern', 'Multi-Region Active-Active', 'Observability', 'FinOps'],
  },
]
  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '2.5rem 1.25rem', textAlign: 'center' }}>
      ...
      <h1 style={{ color: 'var(--text-primary)', fontSize: '2rem', fontWeight: 700, margin: '0 0 0.5rem' }}>
        System Architecture Interview Quiz
      </h1>
      <p style={{ color: 'var(--text-muted)', fontSize: '1rem', marginBottom: '2.5rem' }}>
        30 questions across 3 levels — from Docker and CI/CD basics to Kubernetes, GitOps, chaos engineering, and multi-region deployments.
        Each answer shows the <em>deployment effect</em> and the production best practice.
      </p>
      ...
      <p style={{ color: 'var(--text-muted)', fontSize: '0.78rem', marginTop: '2.5rem' }}>
        Based on Kubernetes Docs, The DevOps Handbook &amp; common backend interview questions · Keyboard: 1–4 to answer · Enter/→ to advance · Tab to switch tabs
      </p>
    </div>
  )
}

// ── Shell/YAML Code block ─────────────────────────────────────────────────────

type TType = 'kw' | 'cmt' | 'str' | 'num' | 'txt'

const SHELL_KW = new Set([
  'docker', 'kubectl', 'helm', 'git', 'curl', 'export', 'echo', 'if', 'then',
  'else', 'fi', 'for', 'do', 'done', 'while', 'case', 'esac', 'function',
  'return', 'true', 'false', 'null', 'yes', 'no', 'run', 'build', 'push',
  'pull', 'apply', 'delete', 'get', 'describe', 'logs', 'exec', 'port-forward',
  'rollout', 'scale', 'deploy', 'service', 'pod', 'deployment', 'namespace',
  'apiVersion', 'kind', 'metadata', 'spec', 'template', 'containers', 'image',
  'name', 'labels', 'selector', 'replicas', 'resources', 'limits', 'requests',
  'memory', 'cpu', 'readinessProbe', 'livenessProbe', 'env', 'ports', 'volume',
])
...
function tokenizeShell(code: string): Token[] {
  ...
  while (i < code.length) {
    ...
    if (/[a-zA-Z_$-]/.test(code[i])) {
      ...
      tokens.push({ t: SHELL_KW.has(w) ? 'kw' : 'txt', v: w })
      i = j
      continue
    }
...
function CodeBlock({ code }: { code: string }) {
  return (
    <pre style={{
      ...
      <code>
        {tokenizeShell(code).map((tok, idx) => (
...
const TABS: { id: ExplanationTab; label: string }[] = [
  ...
  { id: 'compiled', label: 'Deployment Effect' },
          ...
          return (
            <button
              ...
              style={{
                ...
                animation: shaking === i ? 'sysShake 0.55s ease' : 'none',
              }}
            >
              <span style={{
                flexShrink: 0,
            ...
            {state.activeTab === 'compiled' && (
              q.compiledJS ? (
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: '0.88rem', fontStyle: 'italic', margin: 0 }}>
                  This concept operates at the architecture or process level — there is no single deployment snippet that demonstrates it. See the &ldquo;Why This Answer&rdquo; tab for the full explanation.
                </p>
              )
            )}
  ...
  return (
    <div style={{ maxWidth: 680, margin: '0 auto', padding: '2.5rem 1.25rem', textAlign: 'center' }}>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center', flexWrap: 'wrap' }}>
        <button
          onClick={onRetry}
          style={{ background: ACCENT, color: '#fff', border: 'none', borderRadius: '8px', padding: '0.65rem 1.5rem', cursor: 'pointer', fontSize: '0.9rem', fontWeight: 700 }}
        >
          Try Again
        </button>
        <button
...
export function AppSystemArchitectureQuiz() {
  const [state, setState] = useState<QuizState>(INITIAL_STATE)

  const startQuiz = useCallback((level: QuizLevel) => {
    const pool = SYSTEM_ARCHITECTURE_QUESTIONS.filter(q => q.id[0] === level[0])
