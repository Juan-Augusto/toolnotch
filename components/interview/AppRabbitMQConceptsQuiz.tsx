...
import { RABBITMQ_CONCEPTS_QUESTIONS } from '@/data/interview/rabbitmq-concepts-questions'
...
function RmqIcon({ size = 48 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect width="64" height="64" rx="12" fill="#ff6600" />
      {/* Rabbit silhouette */}
      <ellipse cx="32" cy="38" rx="12" ry="10" fill="#fff" fillOpacity="0.9" />
      {/* Left ear */}
      <ellipse cx="25" cy="20" rx="4" ry="9" fill="#fff" fillOpacity="0.9" />
      {/* Right ear */}
      <ellipse cx="39" cy="20" rx="4" ry="9" fill="#fff" fillOpacity="0.9" />
      {/* Inner ears */}
      <ellipse cx="25" cy="20" rx="2.2" ry="6.5" fill="#ff6600" fillOpacity="0.5" />
      <ellipse cx="39" cy="20" rx="2.2" ry="6.5" fill="#ff6600" fillOpacity="0.5" />
      {/* Eye */}
      <circle cx="29" cy="36" r="1.5" fill="#ff6600" />
      {/* Queue dots — conveying "message queue" */}
      <circle cx="20" cy="52" r="2" fill="#fff" fillOpacity="0.7" />
      <circle cx="27" cy="52" r="2" fill="#fff" fillOpacity="0.7" />
      <circle cx="34" cy="52" r="2" fill="#fff" fillOpacity="0.7" />
      <circle cx="41" cy="52" r="2" fill="#fff" fillOpacity="0.7" />
    </svg>
  )
}
...
const LEVEL_CARDS: { level: QuizLevel; title: string; desc: string; topics: string[] }[] = [
  {
    level: 'beginner',
    title: 'Beginner',
    desc: '10 questions on AMQP basics',
    topics: ['AMQP Broker', 'Exchange→Queue→Consumer', 'Bindings', 'Durable Queues', 'Ack / Nack', 'Prefetch Count', 'Default Exchange'],
  },
  {
    level: 'intermediate',
    title: 'Intermediate',
    desc: '10 questions on exchange types and reliability',
    topics: ['Direct Exchange', 'Fanout Exchange', 'Topic Exchange', 'Headers Exchange', 'Routing vs Binding Key', 'DLX', 'Publisher Confirms'],
  },
  {
    level: 'advanced',
    title: 'Advanced',
    desc: '10 questions on internals and production ops',
    topics: ['Channel Multiplexing', 'Flow Control', 'Lazy Queues', 'Quorum Queues', 'Raft Consensus', 'Clustering', 'Monitoring'],
  },
]
  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '2.5rem 1.25rem', textAlign: 'center' }}>
      ...
      <h1 style={{ color: 'var(--text-primary)', fontSize: '2rem', fontWeight: 700, margin: '0 0 0.5rem' }}>
        RabbitMQ Concepts Quiz
      </h1>
      <p style={{ color: 'var(--text-muted)', fontSize: '1rem', marginBottom: '2.5rem' }}>
        30 questions across 3 levels — from AMQP basics and exchange types to clustering, Raft consensus, and production
        monitoring. Each answer shows the broker behaviour and the production best practice.
      </p>
      ...
      <p style={{ color: 'var(--text-muted)', fontSize: '0.78rem', marginTop: '2.5rem' }}>
        Based on RabbitMQ Docs, AMQP 0-9-1 Spec &amp; common backend interview questions · Keyboard: 1–4 to answer · Enter/→ to advance · Tab to switch tabs
      </p>
    </div>
  )
}
...
const SHELL_KW = new Set([
  'curl', 'rabbitmqctl', 'rabbitmqadmin', 'rabbitmq-plugins', 'rabbitmq-diagnostics',
  'GET', 'POST', 'PUT', 'DELETE', 'true', 'false', 'null',
])
...
const TABS: { id: ExplanationTab; label: string }[] = [
  ...
  { id: 'compiled', label: 'Broker Behaviour' },
          ...
          return (
            <button
              ...
              style={{
                ...
                animation: shaking === i ? 'rmqShake 0.55s ease' : 'none',
              }}
            >
              <span style={{
                flexShrink: 0,
            ...
            {state.activeTab === 'compiled' && (
              q.compiledJS ? (
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: '0.88rem', fontStyle: 'italic', margin: 0 }}>
                  This concept applies at the architecture or configuration level — there is no single broker snippet that demonstrates it. See the &ldquo;Why This Answer&rdquo; tab for the full explanation.
                </p>
              )
            )}
...
export function AppRabbitMQConceptsQuiz() {
  const [state, setState] = useState<QuizState>(INITIAL_STATE)

  const startQuiz = useCallback((level: QuizLevel) => {
    const pool = RABBITMQ_CONCEPTS_QUESTIONS.filter(q => q.id[0] === level[0])
