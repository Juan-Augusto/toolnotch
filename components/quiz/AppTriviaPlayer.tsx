...
import AppTriviaResultCard from './AppTriviaResultCard'
...
export default function AppTriviaPlayer({ quiz }: TriviaPlayerProps) {
  ...
  if (state === 'complete' && result) {
    return <AppTriviaResultCard result={result} quiz={quiz} onRetake={retake} />
  }
  ...
  const getOptionClass = (optionId: string): string => {
    ...
    if (!revealed) {
      return `${base} border-gray-200 bg-tertiary text-gray-700 hover:border-blue-300 hover:bg-blue-50/50 dark:border-gray-600 dark:bg-tertiary dark:text-gray-200 dark:hover:border-blue-600 dark:hover:bg-blue-900/20`
    }
    ...
    if (!isSelected && isCorrect) {
      ...
      return `${base} border-green-500 bg-tertiary text-gray-700 dark:bg-tertiary dark:text-gray-200 cursor-not-allowed`
    }
    // Not selected, not correct
    return `${base} border-gray-200 bg-tertiary text-gray-400 dark:border-gray-700 dark:bg-tertiary dark:text-gray-500 cursor-not-allowed`
  }
