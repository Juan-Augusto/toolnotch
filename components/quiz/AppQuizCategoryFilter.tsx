...
export default function AppQuizCategoryFilter({ quizzes }: QuizCategoryFilterProps) {
          ...
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors ${
              activeCategory === cat
                ...
                : 'bg-tertiary border border-gray-200 text-gray-600 hover:border-blue-300 hover:text-blue-600 dark:bg-tertiary dark:border-gray-700 dark:text-gray-300 dark:hover:border-blue-600 dark:hover:text-blue-400'
            }`}
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.map(quiz => (
          <Link
            key={quiz.id}
            href={`/quiz/${quiz.id}`}
            className="bg-tertiary rounded-xl border border-gray-200 shadow-sm p-6 hover:shadow-md transition-shadow hover:border-blue-300 dark:bg-tertiary dark:border-gray-700 dark:hover:border-blue-700 group"
          >
            {/* Category badge */}
            <div className="mb-3">
