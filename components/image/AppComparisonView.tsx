...
export default function AppComparisonView({ original, result }: Props) {
    ...
    <div className="space-y-3">
      ...
      {originalUrl && compressedUrl && (
        ...
        >
          ...
          {/* Divider line */}
          <div
            className="absolute top-0 bottom-0 w-0.5 bg-tertiary dark:bg-zinc-400 shadow-lg pointer-events-none"
          ...
          <div
            ...
            tabIndex={0}
            onKeyDown={onKeyDown}
            className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-8 h-8 bg-tertiary rounded-full shadow-lg border-2 border-gray-300 flex items-center justify-center cursor-col-resize focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600"
            style={{ left: `${sliderPct}%` }}
          >
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
              <path
                d="M5 3L2 7L5 11M9 3L12 7L9 11"
                stroke="#6b7280"
                strokeWidth="1.5"
                strokeLinecap="round"
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        <div className="bg-zinc-50 rounded-lg px-3 py-2 dark:bg-zinc-700">
          <span className="text-zinc-400 text-xs uppercase  block">
            {t("compare.original")}
          </span>
      ...
      >
        {result.savings > 0
          ? t("compare.savings", {
              pct: result.savings,
              bytes: formatBytes(result.originalSize - result.compressedSize),
            })
          : t("compare.noSavings")}
      </div>
