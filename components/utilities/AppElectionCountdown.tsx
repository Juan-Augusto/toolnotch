...
import {
  ...
  getActiveRound,
  getTimeRemaining,
} from "@/lib/electionCountdownMath";
import type {
  CountdownParts,
  ElectionRound,
} from "@/lib/electionCountdownTypes";
...
function Tile({ value, label }: { value: number; label: string }) {
  return (
    <div className="rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 py-4 px-2 text-center">
      ...
      <p className=" text-gray-500 dark:text-gray-400 mt-1 uppercase ">
        {label}
      </p>
    </div>
  );
...
export default function AppElectionCountdown({ locale }: Props) {
  ...
  const roundLabel =
    round.id === "round1" ? t("round1Label") : t("round2Label");
