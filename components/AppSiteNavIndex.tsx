...
import { CATALOG_CATEGORY_ORDER, catalogByCategory } from "@/lib/toolCatalog";

interface Props {
  locale: string;
}

export default function AppSiteNavIndex({ locale }: Props) {
          ...
          return (
            <div key={category}>
              ...
              <ul className="space-y-1">
                {entries.map((e) => (
                  <li key={e.path}>
                    <Link href={`${prefix}${e.path}`} className="link-hover">
