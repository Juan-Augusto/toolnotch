...
export default function AppTermSelector({ value, onChange }: TermSelectorProps) {
      ...
      <div className="flex gap-2 mb-2">
        {PRESETS.map((p) => (
          <button
            ...
            className={`flex-1 py-1.5 text-sm rounded-lg border transition-colors ${value === p.months ? 'bg-blue-600 text-white border-blue-600' : 'bg-tertiary text-gray-600 border-gray-300 hover:bg-gray-50'}`}
          >
            {p.label}
          </button>
        ))}
