...
- **Microtask Queue:** Handles `process.nextTick()` and resolved Promise callbacks (`.then()`). These execute immediately after the current synchronous operation finishes, before the event loop transitions to the next phase.
- **Macrotasks:** Timers (`setTimeout`, `setInterval`) and I/O callbacks run inside their designated event loop phases.
</Callout>

You can also try [Loupe](http://latentflip.com/loupe/?code=%3D!!!PGJ1dHRvbj5DbGljayBtZSE8L2J1dHRvbj4%3D), an interactive visualizer that demonstrates how the event loop processes function calls and asynchronous callbacks in real time.
