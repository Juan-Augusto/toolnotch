...
export const VUE_QUESTIONS: InterviewQuestion[] = [
  ...
  },
  {
    ...
    "bestPractice": "Never use `v-html` with user-supplied content — it bypasses Vue's automatic escaping and is a direct XSS vector. Keep template expressions simple (property access, ternaries); move non-trivial logic into a `computed` property so templates stay readable and the logic is testable in isolation.",
  ...
  },
  {
    ...
    "options": [
      ...
      "It provides a stable identity so Vue can track, reorder, and reuse DOM elements efficiently during diffing",
    ...
    "correctIndex": 1,
    "explanation": "Vue's virtual DOM diffing algorithm, by default, tries to patch existing DOM nodes in place for efficiency. Without a `key`, Vue falls back to in-place patching by index, which can cause it to reuse the wrong DOM node or component instance when the list is reordered, filtered, or has items inserted/removed from the middle — leading to bugs like form inputs retaining stale values or transition animations misfiring. A stable `:key` (typically an item ID, never the array index for mutable lists) lets Vue's diff algorithm match old and new nodes by identity, correctly moving, creating, or destroying only what actually changed.",
    ...
    "bestPractice": "Always use a unique, stable ID from your data as `:key` — never the array index unless the list is strictly static and never reordered/filtered/mutated. When items lack a natural ID, generate one when the data is fetched or created, not on every render (a `Math.random()` key defeats the purpose entirely by changing every re-render).",
  ...
  },
  {
    ...
    "bestPractice": "Use `ref()` for primitives and for values you plan to reassign wholesale (e.g. `data.value = newArray`). Use `reactive()` for objects you will mutate property-by-property. Never destructure a `reactive()` object's properties directly (it loses reactivity) — use `toRefs()` if you need to destructure while preserving reactivity.",
  ...
  },
  {
    ...
    "code": null,
    "options": [
      "Methods cache execution results indefinitely; computed properties recalculate on every component render tick",
      "Computed properties cache their return value based on reactive dependencies, recalculating only when dependencies change",
      "Computed properties execute asynchronously in Web Workers; plain methods execute synchronously on the main thread",
      "There is no difference — template methods and computed properties share identical performance and evaluation rules"
    ],
    "correctIndex": 1,
    "explanation": "A `computed` property is a reactive value derived from other reactive state. Vue tracks exactly which reactive properties it reads during evaluation and caches the result — subsequent accesses return the cached value instantly until one of those tracked dependencies changes, at which point it re-evaluates lazily (on next access, not immediately). A method called in a template (e.g. `{{ getFullName() }}`) re-executes on every single component re-render regardless of whether its inputs changed, because templates re-run the entire render function on each update. For expensive derivations, this caching difference is significant — and it is also the opposite of methods, which cannot take arguments in the caching sense that matters here.",
    ...
    "bestPractice": "Default to `computed` for any value derived purely from other reactive state — it is both faster and more declarative. Reach for a `method` only when the operation has side effects, needs to run on a specific event (like a click), or genuinely needs per-call arguments that vary independently of reactive state.",
  ...
  },
  {
    ...
    "code": null,
    "options": [
      "`ref()` is only for objects and arrays; `reactive()` is required for numbers, strings, and boolean values",
      "`reactive()` returns a Proxy that loses reactivity if reassigned; `ref()` safely allows reassigning `.value`",
      "`ref()` performs deep reactivity tracking; `reactive()` is shallow and watches only top-level object keys",
      "`ref()` works only inside Options API components; `reactive()` is restricted exclusively to Composition API"
    ],
    "correctIndex": 1,
    "explanation": "`reactive()` wraps an object in a Proxy and returns that Proxy directly — property access/writes go through the Proxy traps, giving you ergonomic dot-notation without `.value`. But the reactivity lives in the Proxy object itself: if you do `let state = reactive({ count: 0 }); state = { count: 5 }`, you have just pointed the `state` variable at a brand-new plain object, completely disconnected from Vue's tracking — any template or effect still watching the original Proxy sees no update. `ref()` sidesteps this by always keeping a stable wrapper object `{ value }`; reassigning `.value` is a tracked property write on that stable wrapper, so `myRef.value = newObject` works correctly and triggers updates. This is why many style guides recommend `ref()` even for objects — it is harder to accidentally break.",
    ...
    "bestPractice": "Prefer `ref()` for values that might be reassigned as a whole (fetched API responses, swapped-out arrays) and for primitives. Reserve `reactive()` for object state you will only ever mutate property-by-property and never reassign. Never destructure a `reactive()` object directly — use `toRefs()` first, or the destructured variables lose their reactive connection.",
  ...
  },
  {
    ...
    "options": [
      ...
      "When you need to execute a side effect (API calls, DOM mutations, logging) in response to state changes",
    ...
    "bestPractice": "Ask: \"am I deriving a value to display, or am I reacting to a change with an effect?\" — the former is `computed`, the latter is `watch` (or `watchEffect`). Always clean up side effects a watcher starts (timers, subscriptions) either by returning a cleanup or using `onCleanup` in `watchEffect`, so stale effects do not pile up across re-runs.",
  ...
  },
  {
    ...
    "code": null,
    "options": [
      "It requires passing an explicit array of reactive dependencies as the second argument, identical to React's `useEffect`",
      "It automatically tracks every reactive property read synchronously during callback execution, needing no dependency array",
      "It never re-runs after initial mount — it runs exactly once when the component is created and then automatically disposes",
      "It tracks strictly `ref()` variables, ignoring property changes made on objects wrapped in `reactive()`"
    ],
    "correctIndex": 1,
    "explanation": "`watchEffect` runs its callback immediately (unlike `watch`, which is lazy by default) and, during that run, Vue automatically records every reactive property the function read — no explicit dependency array required, similar in spirit to how `computed` auto-tracks, but for side effects instead of pure derivation. On any subsequent change to any tracked dependency, the whole function re-runs and dependency tracking happens again (so conditionally-accessed dependencies can change between runs, unlike `computed`'s implicit stability). This automatic tracking is convenient but also a footgun: if a dependency is only read inside an `if` branch that is not taken on the first run, it will not be tracked until it actually gets read.",
    ...
    "bestPractice": "Use `watchEffect` for effects whose dependencies are naturally discovered by reading them (e.g. syncing several related values to an API call). Use explicit `watch(source, fn)` when you need the old value, want lazy execution (no immediate run), or want to watch a specific property without accidentally tracking everything else the function happens to touch.",
  ...
  },
  {
    ...
    "bestPractice": "Use string constants (or better, Symbols) as injection keys shared via a dedicated file to avoid typos and key collisions across a large app. Provide a default value as inject's second argument so components can still render meaningfully outside the expected provider tree (useful for component libraries and testing in isolation).",
  ...
  },
  {
    ...
    "options": [
      ...
      "The default slot captures untargeted content, while named slots provide distinct injection points via <template #name>",
  ...
  },
  {
    ...
    "code": null,
    "options": [
      "It lazy-loads external component script bundles on demand when they scroll into the visible browser viewport area",
      "It renders component markup into a different physical DOM node while preserving its reactive scope and logical component tree",
      "It handles server-side rendering hydration reconciliation between node.js output and client virtual DOM trees",
      "It provides route transition animations when navigating between pages in Single Page Applications built with Vue Router"
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "code": null,
    "options": [
      "It forces components to remain permanently rendered in the DOM, toggling their CSS visibility with `display: none`",
      "It caches inactive component instances in memory rather than unmounting them, preserving their state when switched back",
      "It stores component reactive state into browser `localStorage` to survive full page reloads and browser restarts",
      "It prevents child components from re-rendering when their parent component state changes, acting like React.memo"
    ],
    "correctIndex": 1,
    "explanation": "By default, when a dynamic component (via `<component :is=\"...\">`) or a `v-if`-toggled component is switched away from, Vue destroys the old instance entirely — losing all local state, scroll position, and form input values, and running `unmounted` cleanup. `<KeepAlive>` intercepts this: instead of destroying, it deactivates the component (removing it from the DOM but keeping the instance alive in memory) and caches it. Switching back reactivates the cached instance, restoring exactly the state it had when deactivated, and firing `activated`/`deactivated` hooks instead of the full `mounted`/`unmounted` cycle. This is commonly used for tabbed interfaces where switching tabs should preserve scroll position and form state, or multi-step wizards where going back to a previous step should not reset it.",
  ...
  },
  {
    ...
    "options": [
      ...
      "A function using Composition API primitives to encapsulate and return reusable stateful reactive logic",
  ...
  },
  {
    ...
    "code": null,
    "options": [
      "They attach dynamic CSS utility classes to elements; `mounted` is the only lifecycle hook supported by directive APIs",
      "They encapsulate reusable low-level DOM manipulation logic; `mounted(el, binding)` fires once the element enters the DOM",
      "Custom directives were completely deprecated in Vue 2 and replaced by Composition API hooks throughout modern Vue 3",
      "They apply exclusively to custom Vue component root tags and are rejected when declared on native HTML markup elements"
    ],
    "correctIndex": 1,
    "explanation": "Custom directives (`v-my-directive`) exist for cases where you need direct, low-level access to a DOM element that reusable component abstractions do not fit well — auto-focusing an input, implementing a click-outside handler, lazy-loading images on intersection, or integrating a non-Vue-aware DOM library at the element level rather than the component level. A directive definition is an object with lifecycle hooks that mirror component lifecycle timing: `created`, `beforeMount`, `mounted`, `beforeUpdate`, `updated`, `beforeUnmount`, `unmounted` — each receiving the raw DOM element (`el`) and a `binding` object containing the directive's bound value, arguments, and modifiers. `mounted` is the hook that runs once the element actually exists in the DOM, making it the right place for `el.focus()` or attaching a native event listener that needs a real element.",
    "compiledJS": "// v-focus custom directive\nconst vFocus = {\n  mounted(el, binding) {\n    el.focus()\n  },\n}\n\n// Registered locally in <script setup>:\n<script setup>\nconst vFocus = { mounted: (el) => el.focus() }\n</script>\n<template>\n  <input v-focus placeholder=\"Auto-focused on mount\" />\n</template>\n\n// Registered globally (main.js):\napp.directive('focus', {\n  mounted(el) { el.focus() },\n})\n// usable anywhere as v-focus without importing",
    "bestPractice": "Reserve custom directives for genuine low-level DOM concerns — most reusable UI logic is better expressed as a component or a composable, both of which compose more predictably and are easier to test than directives. Always clean up anything a directive sets up in `mounted` (event listeners, observers) inside the matching `unmounted` hook to avoid leaks.",
  ...
  },
  {
    ...
    "options": [
      ...
      "Pinia provides devtools time-travel, SSR-safe store instances per request, and native TypeScript inference",
  ...
  },
  {
    ...
    "options": [
      ...
      "It only tracks reactivity on .value reassignment, skipping deep reactive conversion of nested properties",
  ...
  },
  {
    ...
    "code": null,
    "options": [
      "It represents a benign dev-mode warning caused by CSS class names, having zero consequence on production data or events",
      "Client-rendered Virtual DOM mismatches server-rendered HTML markup, causing Vue to discard HTML and re-render from scratch",
      "It indicates that Vue Router failed to match the incoming request URL against registered client routing table entries",
      "It occurs exclusively when Pinia stores fail to serialize state into script tags during server side rendering pipelines"
    ],
    "correctIndex": 1,
