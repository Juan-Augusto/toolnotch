...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'interview.databaseIndexing' })
  ...
  return {
    ...
    keywords: [
      ...
      "partial index database quiz",
      "EXPLAIN ANALYZE interview questions",
      "SQL query optimization quiz"
    ],
    openGraph: {
      title,
      description,
...
export default async function DatabaseIndexingQuizPage({ params }: Props) {
  ...
  const jsonLd = buildJsonLd(
    interviewQuizSchema({
      ...
      url: localizedUrl,
      locale,
      about: 'Database Indexing & Query Optimization',
      questions,
    }),
    faqSchema(depthFaqs),
    breadcrumbSchema([
      ...
      { name: 'Database Indexing', url: localizedUrl },
    ]),
  )
  ...
  return (
    <main className="min-h-[calc(100vh-180px)] bg-background text-foreground py-8">
      ...
      <div className="w-full max-w-4xl mx-auto pt-4 pb-8 md:pb-12 px-4 sm:px-0">
        <AppDatabaseIndexingQuiz locale={locale} />
      </div>
      ...
      <div className="w-full max-w-4xl mx-auto px-4 sm:px-0">
        <AppInterviewDepth
          ...
          faqs={depthFaqs}
        />
        <AppRelatedInterviewDrills currentSlug="database-indexing" locale={locale} />
      </div>
    </main>
  )
}
