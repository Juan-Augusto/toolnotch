...
interface CensorImageContentProps {
  richContent?: RichContent;
  faqs: FaqItem[];
  locale: string;
}

export default function CensorImageContent({
  richContent,
  faqs,
  locale,
}: CensorImageContentProps) {
  ...
  const relatedTools = [
    {
      ...
      desc:
        locale === "pt"
          ? "Corte partes indesejadas e ajuste fotos em proporções livres e fixas"
          : locale === "es"
            ? "Recorta partes innecesarias y encuadra fotos en proporciones libres o fijas"
            : "Crop unwanted areas and frame photos with free and fixed aspect ratios",
    ...
    },
    {
      ...
      desc:
        locale === "pt"
          ? "Altere a resolução por pixels ou presets de redes sociais"
          : locale === "es"
            ? "Cambia la resolución por píxeles o plantillas de redes sociales"
            : "Change image resolution by pixels or social media presets",
    ...
    },
    {
      ...
      desc:
        locale === "pt"
          ? "Reduza o peso de imagens mantendo transparência com alta fidelidade"
          : locale === "es"
            ? "Reduce el tamaño manteniendo transparencia con máxima calidad"
            : "Reduce file sizes while preserving transparency and sharp quality",
        ...
        <article className="space-y-6 sm:space-y-8 md:space-y-10 mb-6 sm:mb-8 md:mb-10 max-w-4xl mx-auto">
          ...
          {richContent.whatIs && (
            <section aria-labelledby="what-is-heading">
              ...
              >
                {locale === "pt"
                  ? "O que é Censura e Desfoque de Imagem?"
                  : locale === "es"
                    ? "¿Qué es Censurar y Desenfocar una Imagen?"
                    : "What is Image Censoring & Blurring?"}
              </h2>
              <p className="leading-relaxed text-label font-mono text-xs sm:text-sm">
                {richContent.whatIs}
              </p>
          ...
          {richContent.whyItMatters && (
            <section aria-labelledby="why-it-matters-heading">
              ...
              >
                {locale === "pt"
                  ? "Por que Proteger Dados Sensíveis em Imagens?"
                  : locale === "es"
                    ? "¿Por Qué Proteger Datos Confidenciales en Imágenes?"
                    : "Why Protect Sensitive Data in Images?"}
              </h2>
              <p className="leading-relaxed text-label font-mono text-xs sm:text-sm">
                {richContent.whyItMatters}
              </p>
