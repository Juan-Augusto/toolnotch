...
interface ConvertWebpToPngResultProps {
  originalSize: number;
...
export default function ConvertWebpToPngResult({
  originalSize,
  convertedSize,
  width,
  height,
  ...
  locale,
  t,
}: ConvertWebpToPngResultProps) {
  const outFileName = fileName.replace(/\.webp$/i, "") + ".png";
    ...
    <div className="space-y-6 animate-fade-in" aria-live="polite">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 font-mono">
        <div className="p-4 bg-background border border-border rounded-[2px] flex flex-col justify-between">
          ...
          <span className="text-xs text-label">WebP</span>
        </div>
      ...
      {/* Pré-visualização do PNG Convertido */}
