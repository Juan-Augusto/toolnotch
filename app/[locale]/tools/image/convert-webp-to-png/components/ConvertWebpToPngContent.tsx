...
interface ConvertWebpToPngContentProps {
  richContent?: RichContent;
  faqs: FaqItem[];
  locale: string;
}

export default function ConvertWebpToPngContent({
  richContent,
  faqs,
  locale,
}: ConvertWebpToPngContentProps) {
  ...
  const relatedTools = [
    {
      ...
      desc:
        locale === "pt"
          ? "Converta imagens PNG para WebP e reduza até 80% do peso para web"
          : locale === "es"
            ? "Convierte imágenes PNG a WebP y ahorra hasta un 80% de tamaño web"
            : "Convert PNG files to WebP and save up to 80% size for web delivery",
    ...
    },
    {
      ...
      desc:
        locale === "pt"
          ? "Altere a resolução por pixels com proporção travada ou presets"
          : locale === "es"
            ? "Ajusta la resolución por píxeles o medidas de redes sociales"
            : "Change image dimensions with locked aspect ratio or presets",
    ...
    },
    {
      ...
      desc:
        locale === "pt"
          ? "Recorte fotos em proporções fixas ou personalizadas"
          : locale === "es"
            ? "Encuadra fotos en proporciones fijas o personalizadas"
            : "Crop photos with fixed or custom aspect ratios",
        ...
        <article className="space-y-6 sm:space-y-8 md:space-y-10 mb-6 sm:mb-8 md:mb-10 max-w-4xl mx-auto">
          ...
          {richContent.whatIs && (
            <section aria-labelledby="what-is-heading">
              ...
              >
                {locale === "pt"
                  ? "O que é a Conversão de WebP para PNG?"
                  : locale === "es"
                    ? "¿Qué es la Conversión de WebP a PNG?"
          ...
          {richContent.whyItMatters && (
            <section aria-labelledby="why-it-matters-heading">
              ...
              >
                {locale === "pt"
                  ? "Por que o Formato PNG é Necessário?"
                  : locale === "es"
                    ? "¿Por qué es Necesario el Formato PNG?"
                    : "Why is the PNG Format Needed?"}
              </h2>
              <p className="leading-relaxed text-label font-mono text-xs sm:text-sm">
                {richContent.whyItMatters}
              </p>
