...
import type { RichContent } from "./components/PdfToTextContent";

const PATH = "/tools/pdf/pdf-to-text";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "pdf.pdfToText" });
  ...
  const keywords =
    locale === "pt"
      ? [
          ...
          "copiar texto de pdf protegido",
          "leitor de texto pdf online",
          "ferramentas pdf",
        ]
      : locale === "es"
        ? [
...
export default async function PdfToTextPage({ params }: Props) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "pdf.pdfToText" });
