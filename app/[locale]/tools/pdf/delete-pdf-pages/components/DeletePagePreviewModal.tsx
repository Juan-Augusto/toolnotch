...
interface DeletePagePreviewModalProps {
  file: File | null;
  page: PagePreviewItem | null;
  currentIndex: number;
  totalPages: number;
  markedForDeletion: boolean;
  onToggleDeletion: (pageNumber: number) => void;
  onClose: () => void;
  onPrev: () => void;
  onNext: () => void;
  hasPrev: boolean;
...
export default function DeletePagePreviewModal({
  file,
  page,
  currentIndex,
  totalPages,
  ...
  hasNext,
  t,
}: DeletePagePreviewModalProps) {
  const cacheKey = file && page ? `${file.name}-${page.pageNumber}` : "";
  ...
  // Fetch high-res version of current page if not in cache
  useEffect(() => {
    if (!page || !file) return;

    const key = `${file.name}-${page.pageNumber}`;
    if (highResCache.has(key)) return;

    let isMounted = true;

    renderPdfPage(file, page.pageNumber, 2)
      .then((url) => {
        if (isMounted) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "Escape") {
        ...
        e.preventDefault();
        if (page) onToggleDeletion(page.pageNumber);
      } else if (e.key === "+" || e.key === "=") {
        e.preventDefault();
  ...
  }, [hasPrev, hasNext, onPrev, onNext, onClose, onToggleDeletion, page]);
      ...
      <header className="flex items-center justify-between px-2.5 sm:px-4 py-2 sm:py-3 bg-tertiary border-b border-border text-foreground shrink-0 shadow-xs">
        <div className="flex items-center gap-3">
          <div className="flex flex-col sm:flex-row sm:items-center sm:gap-2.5">
            ...
            <span
              className={`text-[10px] sm:text-xs font-semibold px-2 py-0.5 rounded-[2px] inline-flex items-center gap-1 shrink-0 ${
                markedForDeletion
            ...
            >
              {markedForDeletion ? (
                <>
                  ...
                  <span>{t("previewModal.markedBadge")}</span>
                </>
              ) : (
                <>
                  <Check className="w-3 h-3 shrink-0" />
                  <span>{t("previewModal.keepBadge")}</span>
                </>
              )}
            </span>
          </div>
        ...
        >
          ...
          >
            <img
              ...
              alt={t("card.pageLabel", { number: page.pageNumber })}
              className={`max-h-[calc(100vh-180px)] max-w-[calc(100vw-80px)] sm:max-w-[calc(100vw-140px)] object-contain transition-opacity duration-200 ${
                markedForDeletion ? "opacity-40 grayscale" : "opacity-100"
              }`}
              draggable={false}
            />
      ...
      <footer className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 bg-tertiary border-t border-border shrink-0 text-foreground">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-semibold rounded-[2px] transition-colors cursor-pointer border ${
              markedForDeletion
                ? "bg-secondary text-background border-secondary hover:opacity-90"
                : "bg-red-500/10 hover:bg-red-500/20 text-red-600 border-red-500/30"
            }`}
            title={
              markedForDeletion
                ? t("previewModal.keepPage")
                : t("previewModal.markForDeletion")
            }
          >
            {markedForDeletion ? (
              <>
                <Check className="w-3.5 h-3.5" />
                <span>{t("previewModal.keepPage")}</span>
              </>
            ) : (
              <>
                <Trash2 className="w-3.5 h-3.5" />
                <span>{t("previewModal.markForDeletion")}</span>
              </>
            )}
          </button>
        </div>
