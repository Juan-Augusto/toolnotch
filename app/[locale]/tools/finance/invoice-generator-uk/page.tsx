...
const PATH = '/tools/finance/invoice-generator-uk'

interface Props { params: Promise<{ locale: string }> }

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'finance.invoiceGenerator.invoiceUk' })
  return {
    title: t('metaTitle'),
...
export default async function InvoiceGeneratorUkPage({ params }: Props) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'finance.invoiceGenerator.invoiceUk' })
