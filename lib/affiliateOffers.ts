...
const OFFERS_BY_PATH: Record<string, string[]> = {
  ...
  "/tools/text/reading-time-calculator": ["quillbot-paraphraser"],
...
export function getAffiliateOffers(barePath: string): AffiliatePartner[] {
  ...
  let keys = OFFERS_BY_PATH[normalized];

  if (!keys && normalized.startsWith("/tools/text")) {
    keys = ["quillbot-paraphraser"];
  }
